...
function countBy(records: Array<Record<string, string>>, field: string, keys: string[] = []) {
  const initialCounts = Object.fromEntries(keys.map((key) => [key, 0])) as Record<string, number>;
