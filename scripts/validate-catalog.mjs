...
if (process.argv[1] !== undefined && import.meta.url === pathToFileURL(process.argv[1]).href) {
  ...
  const catalogPath = process.argv[2] ?? new URL("../benchmarks/catalog.json", import.meta.url);
  ...
  validateCatalog(catalog, { scenarioIds, validPriorities });

  console.log(`Validated ${catalog.tasks.length} benchmark tasks.`);
}

export function validateCatalog(catalog, { scenarioIds, validPriorities }) {
...
export function validateUniqueTaskIds(tasks) {
