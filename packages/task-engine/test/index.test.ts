...
test("deriveReviewRequired only requires review for unresolved review outcomes", () => {
...
test("buildTaskSnapshot marks pending and changes-requested scenarios as review required", () => {
  const task = {
    id: "bm_review_required",
    title: "Review required task",
    issueSlug: "benchmark-review-required",
  ...
  const baseScenario = {
    ...
    issueSlug: "benchmark-review-required",
    branch: "scenario/review-required",
    baseBranch: "main",
    shipTargetBranch: "integration/live",
    expectedCiStatus: "passed" as const,
    allowSubmit: true,
    allowShip: false,
    allowRollback: false,
    live: {
      ...
      issueNumber: 1,
      pullNumber: 1,
      headBranch: "scenario/review-required"
    },
    notes: "seeded"
  };
  ...
  assert.equal(
    buildTaskSnapshot({
      task,
      scenario: { ...baseScenario, expectedReviewOutcome: "pending" }
    }).reviewRequired,
    true
  );
  assert.equal(
    buildTaskSnapshot({
      task,
      scenario: { ...baseScenario, expectedReviewOutcome: "changes_requested" }
    }).reviewRequired,
    true
  );
});
